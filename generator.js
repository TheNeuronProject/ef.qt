...
const checkNeedsUpdate = ({files, dest, currentVersion}, {verbose}, cb) => {
	...
	fs.readFile(dest, 'utf8', (err, lastGeneratedFile) => {
		...
		if (lastVersion !== currentVersion) {
			if (verbose) console.log(`[V] Last generated ef.qt version ${lastVersion} not match with current version ${currentVersion}, regenerate...`)
			return cb()
		}
		...
		for (let {filePath, source} of files) {
			...
			if (sourceHash !== lastSourceHashMap.get(filePath)) {
				if (verbose) console.log(`[V] Found hash mismatch in \`${filePath}', regenerate...`)
				return cb()
			}
		}

		return console.log('Nothing changed, no need to update.')
	})
}
...
const generate = ({files, dest}, {verbose, dryrun}, cb) => {
	fs.readJson(path.resolve(__dirname, 'package.json'), (err, packageInfo) => {
		...
		checkNeedsUpdate({files, dest, currentVersion: `v${version}`}, {verbose}, () => {
			try {
				const $results = files.map(file => compile(file))
				const outputContent = `// Generated by ef.qt v${version} on ${(new Date()).toDateString()}
${generateSingleFile($results)}`

				if (verbose || dryrun) console.log('[V] Generating header to:', dest)
				if (dryrun) {
...
const fileWalker = ({dir, outFile, ignores}, {verbose, dryrun}, cb) => {
	...
	walker.on('end', () => generate({files, dest}, {verbose, dryrun}, cb))
}

const scanEntry = ({dir = '.', outFile = 'ef.hpp', ignores = [], extraTypeDef = '.eftypedef', callback}, {verbose, dryrun}) => {
	if (verbose || dryrun) {
