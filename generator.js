...
const generateClass = ({className, nameSpace, $customUsings, $data, $refs, $methods, $mountingpoints, $props, $widgets}) => {
	const proto = $widgets[0].type
	return `namespace ef::ui${nameSpace && `::${nameSpace}` || ''} {
	// Custom using
...
const compile = ({className, nameSpace, source}) => {
	...
	try {
		...
		return [generateClass({className, nameSpace, $customUsings, $data, $refs, $methods, $mountingpoints, $props, $widgets}), {className, nameSpace, $includes, $customIncludes}]
	} catch (e) {
...
${generate(files)}`
					...
					fs.outputFile(realOutPath, outputContent, (err) => {
						if (err) return console.error(err)
						console.log('All done.')
